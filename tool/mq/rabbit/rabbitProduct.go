...
func (rabbitProduct *RbMqClient) handleReconnect() {
	for {
		...
		case <-rabbitProduct.coonNotifyClose: // 一般来说是网络断了
			xlog.XLog.Error("网络波动，或者是断网了。。。 " + strconv.Itoa(ReconnectDelay) + "s后进行重连")
		}
	}
}
	...
	defer func() {
		if e := recover(); e != nil {
			if err, ok := e.(error); ok {
				xlog.XLog.Error("connect 发生错误, err = %+v", xerrors.Errorf("%w", err.(error)))
			} else {
				xlog.XLog.Error("connect 发生错误, err = %v", e)
			}
		}
	}()
	conn, err := amqp.Dial(addr)
	if err != nil {
		xlog.XLog.Errorf("RabbitMq连接失败, err = %+v\n", xerrors.Errorf("%w", err))
		return false
	}
	rabbitProduct.connection = conn
	ch, err := rabbitProduct.connection.Channel()
	if err != nil {
		xlog.XLog.Errorf("Channel连接失败, err = %+v\n", xerrors.Errorf("%w", err))
		return false
	}
	defer ch.Close()
...
func (rabbitProduct *RbMqClient) Publish(msg []byte) bool {
	...
	ch, err := rabbitProduct.connection.Channel()
	if err != nil {
		xlog.XLog.Fatalf("channel连接失败, err = %+v\n", xerrors.Errorf("%w", err))
	}
	defer ch.Close()
	if err = ch.Confirm(false); err != nil {
		xlog.XLog.Fatalf("开启确认模式失败, err = %+v\n", xerrors.Errorf("%w", err))
	}
